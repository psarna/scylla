...
static schema_ptr get_table_from_arn(service::storage_proxy& proxy, std::string_view arn) {
    ...
    // NOTE: A properly formatted, but nonexistent arn returns AccessDeniedException,
    // while an incorrectly formatted one returns ValidationException.
    ...
    try {
        ...
        elogger.debug("Error while getting table from arn: {}", std::current_exception());
...
get_extensions_of_table(service::storage_proxy& proxy, schema_ptr schema) {
    sstring req = format("SELECT extensions FROM system_schema.tables WHERE keyspace_name='{}' AND table_name='{}'",
...
static bool is_legal_tag_char(char c) {
    static const std::string_view extra_legal_chars = "+-=._:/";
    return std::isalnum(c) || std::isspace(c) || extra_legal_chars.find(c) != std::string_view::npos;
}
...
static future<> update_tags(const rjson::value& tags, schema_ptr schema, std::unordered_map<sstring, bytes>&& table_extensions, update_tags_action action) {
     const bytes& tags_extension = table_extensions[executor::TAGS_EXTENSION_NAME];
    ...
    for (auto& extension : table_extensions) {
        if (extension.first == executor::TAGS_EXTENSION_NAME) {
            continue;
        }
        serialized_extensions << format("'{}':'{}',", extension.first, extension.second);
    }
    // Updated alternator tags, formatted as a hex blob, prefixed with 0x
    serialized_extensions << "'" << executor::TAGS_EXTENSION_NAME << "':0x";
