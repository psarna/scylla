...
public:
    ...
    put_item_operation(service::storage_proxy& proxy, rjson::value&& request)
        ...
        , _builder(rjson::get(_request, "Item"), schema(), put_or_delete_item::put_item{}) {
        _pk = _builder.pk();
        _ck = _builder.ck();
    ...
    virtual std::optional<mutation> apply(const std::unique_ptr<rjson::value>& previous_item, api::timestamp_type ts) override {
        ...
        return _builder.build(_schema, ts);
    }
...
public:
    ...
    delete_item_operation(service::storage_proxy& proxy, rjson::value&& request)
        : rmw_operation(proxy, std::move(request))
        , _builder(rjson::get(_request, "Key"), schema(), put_or_delete_item::delete_item{}) {
        _pk = _builder.pk();
        _ck = _builder.ck();
        auto return_values = get_string_attribute(_request, "ReturnValues", "NONE");
        if (return_values != "NONE") {
...
class put_or_delete_item_cas_request : public service::cas_request {
    schema_ptr schema;
    std::vector<put_or_delete_item> _builders;
public:
    put_or_delete_item_cas_request(schema_ptr s, std::vector<put_or_delete_item>&& b) :
        schema(std::move(s)), _builders(std::move(b)) { }
    virtual ~put_or_delete_item_cas_request() = default;
    virtual std::optional<mutation> apply(query::result& qr, const query::partition_slice& slice, api::timestamp_type ts) override {
        std::optional<mutation> ret;
        for (put_or_delete_item& builder : _builders) {
            // We assume all these builders have the same partition.
            if (ret) {
                ret->apply(builder.build(schema, ts));
            } else {
                ret = builder.build(schema, ts);
            }
        }
        return ret;
    }
...
static future<> cas_write(service::storage_proxy& proxy, schema_ptr schema, dht::decorated_key dk, std::vector<put_or_delete_item>&& builders, service::client_state& client_state) {
    auto timeout = default_timeout();
    auto read_command = read_nothing_read_command(schema);
    auto op = seastar::make_shared<put_or_delete_item_cas_request>(schema, std::move(builders));
    return proxy.cas(schema, op, read_command, to_partition_ranges(dk),
            {timeout, empty_service_permit(), client_state, client_state.get_trace_state()},
            db::consistency_level::LOCAL_SERIAL, db::consistency_level::LOCAL_QUORUM,
            timeout, timeout).discard_result();
    // We discarded cas()'s future value ("is_applied") because BachWriteItems
...
static future<> do_batch_write(service::storage_proxy& proxy,
        std::vector<std::pair<schema_ptr, put_or_delete_item>> builders,
        service::client_state& client_state,
        stats& stats) {
    if (builders.empty()) {
    ...
    if (write_isolation != rmw_operation::write_isolation::LWT_ALWAYS) {
        ...
        mutations.reserve(builders.size());
...
future<executor::request_return_type> executor::batch_write_item(client_state& client_state, std::string content) {
    ...
    std::vector<std::pair<schema_ptr, put_or_delete_item>> builders;
    builders.reserve(request_items.MemberCount());
    ...
    for (auto it = request_items.MemberBegin(); it != request_items.MemberEnd(); ++it) {
        ...
        for (auto& request : it->value.GetArray()) {
            ...
            if (r_name == "PutRequest") {
                ...
                builders.emplace_back(schema, put_or_delete_item(
                        item, schema, put_or_delete_item::put_item{}));
                auto mut_key = std::make_pair(builders.back().second.pk(), builders.back().second.ck());
    ...
    return do_batch_write(_proxy, std::move(builders), client_state, _stats).then([] () {
...
future<> executor::maybe_create_keyspace(sstring keyspace_name) {
    if (_proxy.get_db().local().has_keyspace(keyspace_name)) {
        return make_ready_future<>();
    }
    return gms::get_up_endpoint_count().then([this, keyspace_name = std::move(keyspace_name)] (int up_endpoint_count) {
        int rf = 3;
