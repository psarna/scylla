...
future<executor::request_return_type> server::handle_api_request(std::unique_ptr<request>&& req) {
    slogger.warn("Request pointer at the beginning: {}", (void*)req.get());
    ...
    slogger.warn("Request pointer before callback {}", (void*)req.get());
    executor::request_return_type ret = co_await callback_it->second(_executor, client_state, trace_state,
            make_service_permit(std::move(units)), std::move(json_request), std::move(req));
    slogger.warn("Request pointer after callback {} {}", bool(req), (void*)req.get());
    co_return ret;
}
