...
private:
    ...
    // staging sstables that still need to generate materialized view updates
    std::unordered_map<uint64_t, sstables::shared_sstable> _sstables_staging_need_mv_update_generation;
...
public:
    ...
    sstables::shared_sstable make_streaming_sstable_for_write(sstring subdir = {});
    inline sstables::shared_sstable make_streaming_staging_sstable() {
...
public:
    ...
    static future<> populate_column_family(distributed<database>& db, sstring sstdir, sstring ks, sstring cf, sstable_is_staging staging = sstable_is_staging::no);
