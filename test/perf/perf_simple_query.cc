...
static void execute_alternator_update_for_key(service::client_state& state, alternator::executor& executor, int64_t sequence) {
    ...
    executor.update_item(state, tracing::trace_state_ptr(), empty_service_permit(), rjson::parse(json)).get();
};
...
static std::vector<perf_result> test_alternator_read(service::client_state& state, noncopyable_function<void()> flush_memtables,
    ...
    return time_parallel([&] {
        ...
        return executor.get_item(state, tracing::trace_state_ptr(), empty_service_permit(), rjson::parse(json)).discard_result();
    }, cfg.concurrency, cfg.duration_in_seconds, cfg.operations_per_shard);
}
...
static std::vector<perf_result> test_alternator_write(service::client_state& state, alternator::executor& executor, test_config& cfg) {
    return time_parallel([&] {
        ...
        return executor.update_item(state, tracing::trace_state_ptr(), empty_service_permit(), rjson::parse(json)).discard_result();
    }, cfg.concurrency, cfg.duration_in_seconds, cfg.operations_per_shard);
}
...
static std::vector<perf_result> do_alternator_test(service::client_state& state,
    ...
    sharded<db::system_distributed_keyspace> dummy_sdks;
    sharded<cdc::metadata> dummy_cdc_metadata;

    smp_service_group_config c;
    c.max_nonlocal_requests = 5000;
    smp_service_group ssg = create_smp_service_group(c).get0();
    auto& proxy = qp.local().proxy().container();
    auto flush_memtables = [&proxy] {
        std::cout << "Flushing partitions..." << std::endl;
        proxy.local().get_db().invoke_on_all(&database::flush_all_memtables).get();
    };
    dummy_sdks.start(std::ref(qp), std::ref(mm), std::ref(proxy)).get();
    dummy_cdc_metadata.start().get();
    executor.start(std::ref(proxy),
            std::ref(mm),
            // parameters below are only touched by alternator streams;
            //  not really interesting for this use case
            std::ref(dummy_sdks),
            std::ref(service::get_storage_service()),
            std::ref(dummy_cdc_metadata),
            // end-of-streams-parameters
            ssg).get();
    // Use LWT only for read-modify-write requests
    // TODO: allow configuring custom write isolation modes
    alternator::rmw_operation::set_default_write_isolation("o");
    std::cout << "Started alternator executor" << std::endl;
    auto stop_sdks = defer([&] {
        dummy_sdks.stop().get();
    });
    auto stop_cdc = defer([&] {
        dummy_cdc_metadata.stop().get();
    });
    auto stop_alternator = defer([&] {
        executor.stop().get();
    });

    try {
        create_alternator_table(state, executor.local());

        switch (cfg.mode) {
        case test_config::run_mode::read:
            return test_alternator_read(state, std::move(flush_memtables), executor.local(), cfg);
        case test_config::run_mode::write:
            return test_alternator_write(state, executor.local(), cfg);
        case test_config::run_mode::del:
            return test_alternator_delete(state, std::move(flush_memtables), executor.local(), cfg);
        };
...
int main(int argc, char** argv) {
    ...
    app.add_options()
        ...
        ("alternator", "use alternator frontend instead of CQL")
        ;
        ...
        return smp::invoke_on_all([seed] {
          ...
          return do_with_cql_env_thread([&app] (auto&& env) {
            ...
            auto results = cfg.frontend == test_config::frontend_type::cql
                    ? do_cql_test(env, cfg)
                    : do_alternator_test(env.local_client_state(), env.qp(), env.migration_manager(), cfg);
