...
void set_failure_injector(http_context& ctx, routes& r) {
    hf::set_breakpoint.set(r, [&ctx](std::unique_ptr<request> req) {
        sstring breakpoint = req->get_query_param("breakpoint");
        ...
        return ctx.db.invoke_on_all([breakpoint, failure_handler, failure_args, count, delay] (database& local_db) {
            local_db.get_failure_injector().register_failure_for(breakpoint, failure_handler, failure_args, count, delay);
        }).then([] {
    ...
    hf::get_active_breakpoints.set(r, [&ctx](std::unique_ptr<request> req) {
        return ctx.db.map_reduce0([] (database& db) {
            return db.get_failure_injector().get_active_breakpoints();
        }, std::vector<sstring>(),
            [](std::vector<sstring> a, std::vector<sstring>&& b) mutable {
            for (auto&& x : b) {
    ...
    hf::unset_breakpoint.set(r, [&ctx](std::unique_ptr<request> req) {
        sstring breakpoint = req->get_query_param("breakpoint");

        return ctx.db.invoke_on_all([breakpoint] (database& local_db) {
            local_db.get_failure_injector().unregister_failure_for(breakpoint);
        }).then([] {
            return make_ready_future<json::json_return_type>(json::json_void());
        });
    });
