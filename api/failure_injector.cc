...
void set_failure_injector(http_context& ctx, routes& r) {
    if (!debug::failure_injector::statically_enabled) {
        throw httpd::bad_param_exception("Failure injection disabled");
    }

    hf::set_breakpoint.set(r, [](std::unique_ptr<request> req) {
        sstring breakpoint = req->param["breakpoint"];
        ...
        return smp::invoke_on_all([breakpoint, failure_handler, failure_args, count, delay] {
            debug::get_failure_injector().register_failure_for(breakpoint, failure_handler, failure_args, count, delay);
        }).then([] {
    ...
    hf::get_active_breakpoints.set(r, [](std::unique_ptr<request> req) {
        return seastar::map_reduce(smp::all_cpus(), [] (unsigned id) {
            return make_ready_future<std::vector<sstring>>(debug::get_failure_injector().get_active_breakpoints());
        }, std::vector<sstring>(),
    ...
    hf::unset_breakpoint.set(r, [](std::unique_ptr<request> req) {
        sstring breakpoint = req->param["breakpoint"];

        return smp::invoke_on_all([breakpoint] {
            debug::get_failure_injector().unregister_failure_for(breakpoint);
        }).then([] {
            return make_ready_future<json::json_return_type>(json::json_void());
        });
    });
