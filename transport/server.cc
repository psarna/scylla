...
future<> cql_server::connection::process_request() {
    return read_frame().then_wrapped([this] (future<std::optional<cql_binary_frame_v3>>&& v) {
        ...
        const bool interactive = _client_state.get_workload_type() == service::client_state::workload_type::interactive;
        if (interactive && _shed_incoming_requests) {
        ...
        if (_server._memory_available.waiters()) {
            if (interactive && !_shedding_timer.armed()) {
