...
future<> controller::do_start_server() {
    ...
    return seastar::async([this] {
        ...
        cql_server_config.max_concurrent_requests = cfg.max_concurrent_cql_requests();
        _observers.start().get();
        _observers.invoke_on_all([this, cserver] (std::vector<std::any>& observers) {
            auto& cfg = _db.local().get_config();
            observers.emplace_back(make_lw_shared<utils::observer<uint64_t>>(cfg.max_concurrent_cql_requests.observe([cserver] (size_t value) {
                logger.info("Setting max concurrent CQL requests threshold to {}", value);
                cserver->local().set_max_concurrent_requests(value);
            })));
        }).get();
