...
std::optional<atomic_cell_view> collection_type_impl::deserialize_single_cell(bytes_view in, bytes_view wanted_key) const {
    ...
    for (uint32_t i = 0; i != nr; ++i) {
        ...
        if (key == wanted_key) {
            return atomic_cell_view::from_bytes(value_comparator()->imr_state().type_info(), read_simple_bytes(in, vsize));
        } else {
