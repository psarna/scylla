...
column_definition::column_definition(bytes name, data_type type, column_kind kind, column_id component_index, column_view_virtual is_view_virtual, column_computed computed, api::timestamp_type dropped_at)
        : _name(std::move(name))
        , _dropped_at(dropped_at)
        , _is_atomic(type->is_atomic())
        , _is_counter(type->is_counter())
        , _is_view_virtual(is_view_virtual)
        , _is_computed(computed)
        , type(std::move(type))
        , id(component_index)
        , kind(kind)
{}
...
schema_builder& schema_builder::with_column(const column_definition& c) {
    return with_column(bytes(c.name()), data_type(c.type), column_kind(c.kind), c.position(), c.view_virtual(), c.computed());
}
...
schema_builder& schema_builder::with_column(bytes name, data_type type, column_kind kind, column_id component_index, column_view_virtual is_view_virtual, column_computed is_computed) {
    _raw._columns.emplace_back(name, type, kind, component_index, is_view_virtual, is_computed);
...
schema_builder& schema_builder::with_computed_column(bytes name, data_type type, column_kind kind) {
    return with_column(name, type, kind, 0, column_view_virtual::no, column_computed::yes);
}
...
schema_builder& schema_builder::mark_column_computed(bytes name) {
    auto it = boost::find_if(_raw._columns, [&name] (auto& c) { return c.name() == name; });
    assert(it != _raw._columns.end());
    it->mark_computed();
