...
column_definition::column_definition(bytes name, data_type type, column_kind kind, column_id component_index, column_view_virtual is_view_virtual, std::optional<column_computed_info> computed, api::timestamp_type dropped_at)
        : _name(std::move(name))
        , _dropped_at(dropped_at)
        , _is_atomic(type->is_atomic())
        , _is_counter(type->is_counter())
        , _is_view_virtual(is_view_virtual)
        , _computed_info(computed)
        , type(std::move(type))
        , id(component_index)
        , kind(kind)
{}
...
schema_builder& schema_builder::with_column(const column_definition& c) {
    return with_column(bytes(c.name()), data_type(c.type), column_kind(c.kind), c.position(), c.view_virtual(), c.get_computed_info());
}
...
schema_builder& schema_builder::with_column(bytes name, data_type type, column_kind kind, column_id component_index, column_view_virtual is_view_virtual, std::optional<column_computed_info> computed) {
    _raw._columns.emplace_back(name, type, kind, component_index, is_view_virtual, std::move(computed));
...
schema_builder& schema_builder::with_computed_column(bytes name, data_type type, column_kind kind, column_computed_info info) {
    return with_column(name, type, kind, 0, column_view_virtual::no, info);
}
...
schema_builder& schema_builder::mark_column_computed(bytes name, column_computed_info computed) {
    auto it = boost::find_if(_raw._columns, [&name] (auto& c) { return c.name() == name; });
    assert(it != _raw._columns.end());
    it->set_computed(computed);
...
column_computed_info column_computed_info::deserialize(bytes raw) {
    if (raw == bytes("token")) {
        return {token};
    }
    throw std::runtime_error("Incorrect column computed info value");
}

bytes column_computed_info::serialize() const {
    switch (type) {
    case token:
        return bytes("token");
    default:
