...
alterSessionStatement returns [std::unique_ptr<alter_session_statement> stmt]
    : K_ALTER K_SESSION K_SET k=cident '='
        ( c=constant { $stmt = std::make_unique<alter_session_statement>(k->to_string(), c->get_raw_text()); }
        | K_NULL  { $stmt = std::make_unique<alter_session_statement>(k->to_string(), sstring("null")); } )
    ;
