...
class selectable::with_map_value_selection : public selectable {
public:
    shared_ptr<selectable> _selected;
    ::shared_ptr<constants::value> _key;
public:
    with_map_value_selection(shared_ptr<selectable> selected, ::shared_ptr<constants::value> key)
            : _selected(std::move(selected)), _key(std::move(key)) {
    }
    ...
    class raw : public selectable::raw {
        ...
        ::shared_ptr<constants::literal> _key;
    public:
        raw(shared_ptr<selectable::raw> selected, ::shared_ptr<constants::literal> key)
                : _selected(std::move(selected)), _key(std::move(key)) {
        }
