...
static future<> delete_ghost_rows(dht::partition_range_vector&& partition_ranges, std::vector<query::clustering_range>&& clustering_bounds, view_ptr view,
    ...
    return seastar::async([partition_ranges = std::move(partition_ranges), clustering_bounds = std::move(clustering_bounds), view = std::move(view), &proxy, &state, &options, &stats, timeout_duration] {
        ...
        while (!p->is_exhausted()) {
            tracing::trace(state.get_trace_state(), "Fetching a base table page for ghost row deletion");
