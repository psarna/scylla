...
{
    static std::string_view legal_values[] = {"read_timeout", "write_timeout"};
    if (std::find(std::begin(legal_values), std::end(legal_values), _key) == std::end(legal_values)) {
        throw exceptions::invalid_request_exception(format("Unknown session-specific param: {}", _key));
    }
}
...
get_params(const std::map<sstring, sstring>& raw_params) {
    ...
    auto reads = raw_params.find("read_timeout");
    auto writes = raw_params.find("write_timeout");
    ...
    if (reads != raw_params.end()) {
	    params.read_timeout = get_duration(reads->second);
    }
    if (writes != raw_params.end()) {
	    params.write_timeout = get_duration(writes->second);
    }
    return params;
}
