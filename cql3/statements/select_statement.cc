...
select_statement::make_partition_slice(const query_options& options)
{
    ...
    static logging::logger srn2("SARNA222");
    srn2.warn("USING CLUSTERING BOUNDS FROM PREPARED RESTRICTIONS");
    auto bounds =_prepared_restrictions.get_clustering_bounds(options);
    auto boundsorig = _restrictions->get_clustering_bounds(options);
    srn2.warn("BOUNDS_ORIG = {} ; BOUNDS = {}", boundsorig, bounds);
                          ...
                          const query_options& options)
{
   ...
   auto key_ranges_depr = _restrictions->get_partition_key_ranges(options);
    auto key_ranges2 = _prepared_restrictions.get_partition_key_ranges(options);
    srn.warn("Ranges {}; Ranges2: {}", key_ranges_depr, key_ranges2);
    auto it = key_ranges_depr.begin();
    auto it2 = key_ranges2.begin();
    while (it != key_ranges_depr.end()) {
        std::stringstream s1, s2;
        s1 << *it;
        s2 << *it2;
        if (s1.str() != s2.str()) {
            throw std::runtime_error(format("RANGE NOT EQUAL!: {} vs {}", s1.str(), s2.str()));
        }
        ++it;
        ++it2;
    }
    ...
    if (!aggregate && !restrictions_need_filtering && (page_size <= 0
            || !service::pager::query_pagers::may_need_paging(*_schema, page_size,
                    *command, key_ranges2))) {
        return execute(proxy, command, std::move(key_ranges2), state, options, now);
    }

    command->slice.options.set<query::partition_slice::option::allow_short_read>();
    auto timeout_duration = options.get_timeout_config().*get_timeout_config_selector();
    srn.warn("Will create pager with filtering={}", restrictions_need_filtering);
    auto p = service::pager::query_pagers::pager(_schema, _selection,
            state, options, command, std::move(key_ranges2), _stats, restrictions_need_filtering ? _restrictions : nullptr);

    srn.warn("Aggregate {} nonpaged {}", aggregate, nonpaged_filtering);
    if (aggregate || nonpaged_filtering) {
        return do_with(
    ...
    auto timeout = db::timeout_clock::now() + timeout_duration;
    srn.warn("Selection is trivial? {} filtering {} ppl {}", _selection->is_trivial(), restrictions_need_filtering, _per_partition_limit);
                                  ...
                                  gc_clock::time_point now)
{
    ...
    srn.warn("fast path? {}. Filtering? {}", fast_path, restrictions_need_filtering);
    if (fast_path) {
