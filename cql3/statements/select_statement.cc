...
future<shared_ptr<cql_transport::messages::result_message>>
select_statement::do_execute_coroutine(lw_shared_ptr<query::read_command>&& command,
    ...
    if (_selection->is_trivial() && !restrictions_need_filtering && !_per_partition_limit) {
        auto generator = co_await p->fetch_page_generator(page_size, now, timeout, _stats);
    ...
    auto rs = co_await p->fetch_page(page_size, now, timeout);
                          ...
                          const query_options& options) const
{
    ...
    if (db::is_serial_consistency(options.get_consistency())) {
        if (key_ranges.size() != 1 || !query::is_single_partition(key_ranges.front())) {
             return make_exception_future<shared_ptr<cql_transport::messages::result_message>>(exceptions::invalid_request_exception(
        ...
        if (this_shard_id() != shard) {
            ...
            return make_ready_future<shared_ptr<cql_transport::messages::result_message>>(make_shared<cql_transport::messages::result_message::bounce_to_shard>(shard));
        }
    }
    ...
    return do_execute_coroutine(std::move(command), std::move(key_ranges), aggregate,
            nonpaged_filtering, restrictions_need_filtering, page_size, now, proxy, state, options);
}
