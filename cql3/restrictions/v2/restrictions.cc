...
static std::vector<query::range<partition_key>> compute_single_column_pk_bounds_from_slice(const column_definition* def,
    ...
    if (cartesian_product_is_empty(vec_of_values)) {
        auto read_bound = [slice, &options, &schema] (statements::bound b) -> std::optional<range_bound> {
            ...
            rlogger.warn("Ret {}", partition_key::from_single_value(schema, *value), slice->is_inclusive(b));
            return {range_bound(partition_key::from_single_value(schema, *value), slice->is_inclusive(b))};
        };
    ...
    for (auto&& prefix : make_cartesian_product(vec_of_values)) {
        auto read_bound = [slice, &prefix, &options, &schema](statements::bound bound) -> range_bound {
            if (slice->has_bound(bound)) {
                ...
                auto val = partition_key::from_optional_exploded(schema, prefix);
                prefix.pop_back();
                rlogger.warn("Reet {}", std::move(val), slice->is_inclusive(bound));
                return range_bound(std::move(val), slice->is_inclusive(bound));
            } else {
                rlogger.warn("Reeeeet {}", partition_key::from_optional_exploded(schema, prefix));
                return range_bound(partition_key::from_optional_exploded(schema, prefix));
            }
        };
...
static std::vector<query::range<partition_key>> compute_single_column_pk_bounds(const prepared_restrictions& restrictions, const query_options& options) {
    using range_type = query::range<partition_key>;
    std::vector<range_type> ranges;

    const schema& schema = *restrictions._schema;

    auto pk_restrictions = restrictions.pk_restrictions() | boost::adaptors::filtered([] (const restriction& r) { return !r.on_token(); });
    ...
    if (boost::algorithm::all_of(pk_restrictions, [] (const restriction& r) { return r._op == operator_type::EQ; })) {
        ...
        if (!pk_restrictions.empty() && std::next(pk_restrictions.begin()) == pk_restrictions.end()) {
            ...
            const column_definition* cdef = r._target[0]; // FIXME(sarna): Assumes single column restrictions, true for pk, unsafe for future uses
    ...
    for (const restriction& r : pk_restrictions) {
        ...
        if (slice) {
            ...
            return compute_single_column_pk_bounds_from_slice(def, slice, vec_of_values, schema, options);
        }
...
dht::partition_range_vector prepared_restrictions::get_partition_key_ranges(const query_options& options) const {
    ...
    if (token_slice) {
        ...
        for (query::range<partition_key>& r : compute_single_column_pk_bounds(*this, options)) {
            if (!r.is_singular()) {
