...
future<row_locker::lock_holder> table::push_view_replica_updates(const schema_ptr& s, mutation&& m, db::timeout_clock::time_point timeout, exclude_staging_sstables exclude_staging) const {
    ...
    return lockf.then([this, m = std::move(m), slice = std::move(slice), views = std::move(views), base, timeout, exclude_staging] (row_locker::lock_holder lock) mutable {
        ...
        if (exclude_staging) {
            generate_updates = generate_and_propagate_view_updates_without_staging(base, dht::partition_range::make_singular(m.decorated_key()), std::move(slice), std::move(m), std::move(views), timeout);
        } else {
            generate_updates = generate_and_propagate_view_updates(base, dht::partition_range::make_singular(m.decorated_key()), std::move(slice), std::move(m), std::move(views), timeout);
        }
