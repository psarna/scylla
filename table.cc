...
future<> table::generate_and_propagate_view_updates(const schema_ptr& base,
    ...
    return db::view::generate_view_updates(
        ...
        return db::view::mutate_MV(std::move(base_token), std::move(updates), _view_stats, *_config.cf_stats, std::move(units),
                service::allow_hints::yes, db::view::sync_local_view_updates::no).handle_exception([] (auto ignored) { });
    });
}
...
future<> table::populate_views(
    ...
    return db::view::generate_view_updates(
        ...
        return seastar::get_units(*_config.view_update_concurrency_semaphore, units_to_wait_for).then(
            ...
            return db::view::mutate_MV(std::move(base_token), std::move(updates), _view_stats, *_config.cf_stats, std::move(units),
                    service::allow_hints::no, db::view::sync_local_view_updates::no);
        });
    });
}
