...
target_info target_parser::parse(schema_ptr schema, const sstring& target) {
    ...
    auto column_from_json = [&schema] (const Json::Value& parsed_col, std::vector<column_definition>& computed_column_holder) -> const column_definition* {
        ...
        if (parsed_col.isObject()) {
            ...
            computed_column_holder.emplace_back(to_bytes(column_name), values_type, column_kind::regular_column, 0, column_view_virtual::no, std::make_unique<column_computation>(column_computation::deserialize(parsed_col)));
            computed_column_holder.back().init_column_specification(*schema);
            return &computed_column_holder.back();
        }
    ...
    if (is_json && json_value.isObject()) {
        ...
        for (auto it = pk.begin(); it != pk.end(); ++it) {
            info.pk_columns.push_back(column_from_json(*it, info.computed_column_holder));
        }
        for (auto it = ck.begin(); it != ck.end(); ++it) {
            info.ck_columns.push_back(column_from_json(*it, info.computed_column_holder));
        }
