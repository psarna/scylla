...
static bytes get_available_map_value_column_name(const schema& schema, const bytes& root) {
...
target_parser::target_info target_parser::parse(schema_ptr schema, const sstring& target) {
    ...
    auto column_from_json = [&schema] (const Json::Value& parsed_col, std::vector<column_definition>& columns_placeholder) -> const column_definition* {
        ...
        if (parsed_col.isObject()) {
            ...
            srn.warn("WILLPARSE {}", parsed_col.toStyledString());
            auto column_name = get_available_map_value_column_name(*schema, to_bytes(map_name.asString()));
            srn.warn("colname {}, key {} <{}>", column_name, map_key.toStyledString(), map_key.type());
            ...
            auto value_type = collection_type->get_values_type();
            columns_placeholder.emplace_back(to_bytes(column_name), value_type, column_kind::regular_column, 0, column_view_virtual::no, column_computation::deserialize(parsed_col));
...
sstring target_parser::serialize_targets(const std::vector<::shared_ptr<cql3::statements::index_target>>& targets) {
    ...
    struct as_json_visitor {
        ...
        Json::Value operator()(const index_target::map_entry& entry) const {
