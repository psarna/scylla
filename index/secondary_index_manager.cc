...
view_ptr secondary_index_manager::create_view_for_index(const index_metadata& im) const {
    srn.warn("CREATING VIEW FOR INDEX");
    ...
    const column_definition* index_target = im.local() ? target_info.ck_columns.front() : target_info.pk_columns.front();
    ...
    if (im.local()) {
        ...
        builder.with_column(index_target->name(), index_target->type, column_kind::clustering_key, 0, column_view_virtual::no, index_target->get_computation_ptr());
        srn.warn("BUILT local WITH COMPUTATION {} {}", index_target->name(), bool(index_target->is_computed()));
    } else {
        builder.with_column(index_target->name(), index_target->type, column_kind::partition_key, 0, column_view_virtual::no, index_target->get_computation_ptr());
        srn.warn("BUILT WITH COMPUTATION {} {}", index_target->name(), bool(index_target->is_computed()));
    ...
    srn.warn("CREATED VIEW FOR INDEX");
