...
bool index::supports_expression(const column_definition& cdef, const cql3::operator_type& op) const {
    return cdef.name_as_text() == _target_column
            && (op == cql3::operator_type::EQ || op == cql3::operator_type::CONTAINS || op == cql3::operator_type::CONTAINS_KEY);
}
...
view_ptr secondary_index_manager::create_view_for_index(const index_metadata& im) const {
    ...
    if (im.local()) {
        ...
        builder.with_computed_column(token_column_name, bytes_type, column_kind::clustering_key, std::make_unique<column_computation>(token_column_computation()));
        for (auto& col : schema->partition_key_columns()) {
            if (col == *index_target) {
                continue;
            }
