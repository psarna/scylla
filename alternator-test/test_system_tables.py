...
def test_fetch_from_system_tables(scylla_only, dynamodb):
    ...
    system_tables_to_test = ['large_rows', 'large_partitions', 'large_cells', 'tables', 'roles', 'role_members',
        'columns', 'functions', 'keyspaces', 'types', 'built_views', 'scylla_local']

    for item in tables_response['Items']:
        ks_name = item['keyspace_name']
        table_name = item['table_name']

        if not table_name in system_tables_to_test:
            continue
        ...
        col_response = client.query(TableName=internal_prefix+'system_schema.columns',
                ...
                AttributesToGet=['keyspace_name', 'table_name', 'column_name', 'kind'])
