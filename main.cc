            ...
            if (cfg->alternator_port() || cfg->alternator_https_port()) {
                static struct shutdown_alternator : public service::client_shutdown_listener {
                    ...
                    virtual void on_client_shutdown_requested() override {
                        if (engine().cpu_id() == 0) {
                            startlog.info("Shutting down alternator");
                            try {
                                executor.stop().get();
                                server.stop().get();
                            } catch (...) {
                                startlog.error("Unexpected error shutting down alternator: {}", std::current_exception());
                                throw;
                            }
                            startlog.info("Shutting down alternator was successful");
                        }
                    }
