    ...
    return app.run(ac, av, [&] () -> future<int> {
        ...
        return seastar::async([cfg, ext, &db, &qp, &proxy, &mm, &ctx, &opts, &dirs, &pctx, &prometheus_server, &return_value, &cf_cache_hitrate_calculator,
          ...
          try {
            ...
            if (cfg->alternator_port() || cfg->alternator_tls_port()) {
                ...
                std::optional<uint16_t> alternator_tls_port;
                std::optional<tls::credentials_builder> creds;
                if (cfg->alternator_tls_port()) {
                    creds.emplace();
                    alternator_tls_port = cfg->alternator_tls_port();
                ...
                alternator_server.init(addr, alternator_port, alternator_tls_port, creds).get();
                if (alternator_port) {
                    startlog.info("Alternator server listening on {} port {}", addr, *alternator_port);
                }
                if (alternator_tls_port) {
                    startlog.info("Alternator TLS server listening on {} port {}", addr, *alternator_tls_port);
                }
            }
