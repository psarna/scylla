...
SEASTAR_TEST_CASE(test_view_update_generating_writetime) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        const db::view::stats& t_view_stats = e.local_db().find_column_family("ks", "t").get_view_stats();
        const table::stats& mv1_stats = e.local_db().find_column_family("ks", "mv1").get_stats();
        const table::stats& mv2_stats = e.local_db().find_column_family("ks", "mv2").get_stats();

        auto total_t_view_updates = [&t_view_stats] {
            return t_view_stats.view_updates_pushed_local + t_view_stats.view_updates_pushed_remote;
        };
        ...
        eventually([&] {
            ...
            BOOST_REQUIRE_EQUAL(mv1_stats.writes.hist.count, 1);
            BOOST_REQUIRE_EQUAL(mv2_stats.writes.hist.count, 0);
        });
        ...
        eventually([&] {
            ...
            BOOST_REQUIRE_EQUAL(mv1_stats.writes.hist.count, 2);
            BOOST_REQUIRE_EQUAL(mv2_stats.writes.hist.count, 0);
        });
        ...
        eventually([&] {
            ...
            BOOST_REQUIRE_EQUAL(mv1_stats.writes.hist.count, 3);
            BOOST_REQUIRE_EQUAL(mv2_stats.writes.hist.count, 1);
        });
        ...
        eventually([&] {
            ...
            BOOST_REQUIRE_EQUAL(mv1_stats.writes.hist.count, 4); // only one update for creation, update does not generate one
            BOOST_REQUIRE_EQUAL(mv2_stats.writes.hist.count, 2);
        });
        ...
        eventually([&] {
            ...
            BOOST_REQUIRE_EQUAL(mv1_stats.writes.hist.count, 6); // two updates - one for creation, one for updating the TTL
            BOOST_REQUIRE_EQUAL(mv2_stats.writes.hist.count, 4);
        });
    });
}
