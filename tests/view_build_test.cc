...
SEASTAR_TEST_CASE(test_generating_view_updates_optimizations) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        set_t1_cell(updated, "v1", 2);
        set_t1_cell(updated, "v2", 4);
        set_t1_cell(updated, "v3", 4);
        set_t1_cell(existing, "v1", 2);
        set_t1_cell(existing, "v2", 3);
        set_t1_cell(existing, "v3", 4);

        auto view_updates = db::view::generate_view_updates(
                t1,
                std::vector<view_ptr>{view_ptr(t1_v1_idx_index)},
                flat_mutation_reader_from_mutations({updated}),
                flat_mutation_reader_from_mutations({existing})
        ).get0();
        ...
        // Sub-case with updates 3: column is selected and is mv key
        existing = mutation(t3, pk_t3);
        updated = mutation(t3, pk_t3);
        ...
        // Sub-case with updates 4: column is selected as regular
        existing = mutation(t3, pk_t3);
        updated = mutation(t3, pk_t3);
        set_t3_cell(existing, "c", 1, ttl_opt{});
