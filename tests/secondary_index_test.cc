...
SEASTAR_TEST_CASE(test_local_secondary_index) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        const table::stats& index_table_stats = e.local_db().find_column_family("ks", "local_t_v1_index").get_stats();
        BOOST_REQUIRE_EQUAL(index_table_stats.reads.hist.count, 1);
...
SEASTAR_TEST_CASE(test_local_and_global_secondary_index) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        const table::stats& local_index_table_stats = e.local_db().find_column_family("ks", "local_t_v1_index").get_stats();
        const table::stats& global_index_table_stats = e.local_db().find_column_family("ks", "global_t_v1_index").get_stats();

        BOOST_REQUIRE_EQUAL(local_index_table_stats.reads.hist.count, 1);

        auto res = e.execute_cql("select * from t where v1 = 3").get0();
        BOOST_REQUIRE_EQUAL(global_index_table_stats.reads.hist.count, 1);
