...
SEASTAR_TEST_CASE(test_local_secondary_index) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        e.execute_cql("create custom index local_t_v1 on t (v1) using 'com.scylladb.LocalIndex'").get();
        BOOST_REQUIRE_THROW(e.execute_cql("create custom index local_t_v1 on t (p) using 'com.scylladb.LocalIndex").get(), std::exception);
...
SEASTAR_TEST_CASE(test_local_index_paging) {
    return do_with_cql_env_thread([] (auto& e) {
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON tab (v) USING 'com.scylladb.LocalIndex'").get();
        e.execute_cql("CREATE CUSTOM INDEX ON tab (c2) USING 'com.scylladb.LocalIndex'").get();
...
SEASTAR_TEST_CASE(test_local_index_case_sensitive) {
    return do_with_cql_env_thread([] (auto& e) {
        e.execute_cql("CREATE TABLE \"FooBar\" (a int PRIMARY KEY, b int, c int)").get();
        e.execute_cql("CREATE CUSTOM INDEX ON \"FooBar\" (b) USING 'com.scylladb.LocalIndex'").get();
        e.execute_cql("INSERT INTO \"FooBar\" (a, b, c) VALUES (1, 2, 3)").get();
        e.execute_cql("SELECT * from \"FooBar\" WHERE a = 1 AND b = 1").get();

        e.execute_cql("CREATE TABLE tab (a int PRIMARY KEY, \"FooBar\" int, c int)").get();
        e.execute_cql("CREATE CUSTOM INDEX ON tab (\"FooBar\") USING 'com.scylladb.LocalIndex'").get();

        e.execute_cql("INSERT INTO tab (a, \"FooBar\", c) VALUES (1, 2, 3)").get();
        e.execute_cql("SELECT * from tab WHERE a = 1 and \"FooBar\" = 2").get();

        e.execute_cql("CREATE TABLE tab2 (\"FooBar\" int PRIMARY KEY, b int, c int)").get();
        e.execute_cql("CREATE CUSTOM INDEX ON tab2 (b) USING 'com.scylladb.LocalIndex'").get();
        e.execute_cql("INSERT INTO tab2 (\"FooBar\", b, c) VALUES (1, 2, 3)").get();
...
SEASTAR_TEST_CASE(test_local_index_unorthodox_name) {
    return do_with_cql_env_thread([] (auto& e) {
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON tab (\"Comma\\,,\") USING 'com.scylladb.LocalIndex'").get();
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON tab3 (b) USING 'com.scylladb.LocalIndex'").get();
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON tab4 (b) USING 'com.scylladb.LocalIndex'").get();
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON tab5 (\"(b)\") USING 'com.scylladb.LocalIndex'").get();
        e.execute_cql("INSERT INTO tab5 (a, \"(b)\", c) VALUES (1, 2, 3)").get();
        e.execute_cql("SELECT * from tab5 WHERE a = 1 AND \"(b)\" = 1").get();

        e.execute_cql("CREATE TABLE tab6 (\"trailingbacklash\\\" int, b int, c int, d int, primary key ((\"trailingbacklash\\\", b)))").get();
        e.execute_cql("CREATE CUSTOM INDEX ON tab6(c) USING 'com.scylladb.LocalIndex'").get();
...
SEASTAR_TEST_CASE(test_local_index_operations) {
    return do_with_cql_env_thread([] (auto& e) {
        ...
        e.execute_cql("CREATE CUSTOM INDEX ON t (v1) USING 'com.scylladb.LocalIndex'").get();

        // Duplicate index cannot be created, even if it's named
        BOOST_REQUIRE_THROW(e.execute_cql("CREATE CUSTOM INDEX ON t (v1)").get(), exceptions::invalid_request_exception);
        BOOST_REQUIRE_THROW(e.execute_cql("CREATE CUSTOM INDEX named_idx ON t (v1) USING 'com.scylladb.LocalIndex'").get(), exceptions::invalid_request_exception);
        e.execute_cql("CREATE CUSTOM INDEX IF NOT EXISTS named_idx ON t (v1) USING 'com.scylladb.LocalIndex'").get();
        ...
        e.execute_cql("CREATE CUSTOM INDEX named_idx ON t (v1) USING 'com.scylladb.LocalIndex'").get();
