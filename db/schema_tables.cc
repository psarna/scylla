...
static schema_mutations make_table_mutations(schema_ptr table, api::timestamp_type timestamp, bool with_columns_and_triggers)
{
    ...
    //FIXME(sarna): This check needs to be here only as long as we allow non-computed token columns in indexes for backward compatibility.
    const bool use_computed_columns = service::get_local_storage_service().cluster_supports_computed_columns();

    mutation columns_mutation(columns(), pkey);
    mutation_opt computed_columns_mutation;
    if (use_computed_columns) {
        computed_columns_mutation = mutation(computed_columns(), pkey);
    }
    ...
    if (with_columns_and_triggers) {
        for (auto&& column : table->v3().all_columns()) {
            ...
            if (use_computed_columns && column.is_computed()) {
