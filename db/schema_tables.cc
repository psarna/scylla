...
static std::unordered_set<bytes> get_computed_column_names(const schema_mutations& sm);

static std::vector<column_definition> create_columns_from_column_rows(
                const query::result_set& rows, const sstring& keyspace,
                const sstring& table, bool is_super, column_view_virtual is_view_virtual, const std::unordered_set<bytes>& computed_columns);
...
std::vector<const char*> ALL { KEYSPACES, TABLES, SCYLLA_TABLES, COLUMNS, DROPPED_COLUMNS, TRIGGERS, VIEWS, TYPES, FUNCTIONS, AGGREGATES, INDEXES, VIEW_VIRTUAL_COLUMNS, COMPUTED_COLUMNS };
...
schema_ptr create_table_from_mutations(const schema_ctxt& ctxt, schema_mutations sm, std::optional<table_schema_version> version)
{
    ...
    auto computed_columns = get_computed_column_names(sm);
...
static std::unordered_set<bytes> get_computed_column_names(const schema_mutations& sm) {
    ...
    return boost::copy_range<std::unordered_set<bytes>>(
            computed_result.rows() | boost::adaptors::transformed([] (const query::result_set_row& row) {
        return row.get_nonnull<bytes>("column_name_bytes");
    }));
}
...
static std::vector<column_definition> create_columns_from_column_rows(const query::result_set& rows,
                                                               ...
                                                               const std::unordered_set<bytes>& computed_columns)
{
    ...
    for (auto&& row : rows.rows()) {
        ...
        const column_computed is_computed(computed_columns.count(name_bytes) > 0);

        columns.emplace_back(name_bytes, type, kind, position, is_view_virtual, is_computed);
    }
    return columns;
}
...
std::vector<schema_ptr> all_tables() {
    return {
        ...
        views(), indexes(), types(), functions(), aggregates(), view_virtual_columns(), computed_columns()
    };
}
