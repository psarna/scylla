...
static bool should_sync_local_view_updates_for(const schema& schema) {
    auto it = schema.extensions().find(sync_local_view_updates_extension::NAME);
    if (it == schema.extensions().end()) {
        return false;
    }
    auto ext = static_pointer_cast<db::view::sync_local_view_updates_extension>(it->second);
    return ext->enabled();
}
    ...
    for (frozen_mutation_and_schema& mut : view_updates) {
        ...
        if (paired_endpoint) {
            ...
            if (is_endpoint_local && pending_endpoints.empty()) {
                ...
                if (should_sync_local_view_updates_for(*mut.s)) {
