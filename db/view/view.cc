        ...
        if (view_col.is_computed()) {
            for (const column_definition& dependent_col : view_col.get_computation().dependent_columns(base)) {
                ...
                if (base_col && !base_col->is_primary_key()) {
                    vlogger.warn("SARNA: WOULD HAVE PICKED COLUMN {}", base_col->name_as_text());
...
static bool is_partition_key_empty(
    ...
    if (view_schema.partition_key_columns().front().is_computed()) {
        vlogger.warn("SARNA: IS PARTITION KEY EMPTY FOR COMPUTED? {}", !bool(view_schema.partition_key_columns().front().get_computation().compute_value(base, base_key, update)));
...
row_marker view_updates::compute_row_marker(const clustering_row& base_row) const {
    ...
    vlogger.warn("SARNA: COMPUTED? {} COLID {}", bool(computed_col), col_id);
    if (computed_col) {
...
void view_updates::do_delete_old_entry(const partition_key& base_key, const clustering_row& existing, const clustering_row& update, gc_clock::time_point now) {
    ...
    const column_definition* computed_col = _view_info.computed_column_depending_on_base_in_view_pk();
    auto col_id = _view_info.base_non_pk_column_in_view_pk();
    vlogger.warn("SARNA: comp {} col_id {}", bool(computed_col), col_id.value_or(777));
    if (computed_col) {
...
void view_updates::update_entry(const partition_key& base_key, const clustering_row& update, const clustering_row& existing, gc_clock::time_point now) {
    ...
    if (is_partition_key_empty(*_base, *_view, base_key, existing) || !matches_view_filter(*_base, _view_info, base_key, existing, now)) {
        vlogger.warn("CREATING ENTRY {}, {}", base_key, clustering_row::printer(*_base, existing));
        create_entry(base_key, update, now);
        return;
    }
    if (is_partition_key_empty(*_base, *_view, base_key, update) || !matches_view_filter(*_base, _view_info, base_key, update, now)) {
        vlogger.warn("deleting ENTRY {}, {}", base_key, clustering_row::printer(*_base, update));
        do_delete_old_entry(base_key, existing, update, now);
        return;
    }
...
void view_updates::generate_update(
    ...
    const column_definition* computed_col = _view_info.computed_column_depending_on_base_in_view_pk();
    auto col_id = _view_info.base_non_pk_column_in_view_pk();
    vlogger.warn("SARNA: comp {} col_id gen {}", bool(computed_col), col_id.value_or(779));
    ...
    if (computed_col) {
        vlogger.warn("SARNA: serving computed");
        if (existing) {
            update_entry(base_key, update, *existing, now);
        } else {
            create_entry(base_key, update, now);
