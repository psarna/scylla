...
future<utils::chunked_vector<frozen_mutation_and_schema>> view_update_builder::build_some() {
    if (_updates.is_end_of_stream() && _updates.is_buffer_empty()) {
        return make_ready_future<utils::chunked_vector<frozen_mutation_and_schema>>(utils::chunked_vector<frozen_mutation_and_schema>());
    }
...
future<stop_iteration> view_update_builder::on_results() {
    ...
    if (rows_for_view_updates >= max_rows_for_view_updates) {
        return stop();
    }
