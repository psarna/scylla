...
void view_info::initialize_base_dependent_fields(const schema& base) {
    for (auto&& view_col : boost::range::join(_schema.partition_key_columns(), _schema.clustering_key_columns())) {
        if (view_col.is_computed()) {
            ...
            const bool depends = boost::algorithm::any_of(dependent_columns, [&base] (const column_definition& dependent_col) {
                const column_definition* base_col = base.get_column_definition(dependent_col.name());
                return base_col && !base_col->is_primary_key();
            });
            if (depends) {
    ...
    auto get_value = boost::adaptors::transformed([&, this] (const column_definition& cdef) -> bytes_view {
        ...
        if (!base_col) {
            if (!cdef.is_computed()) {
                ...
                auto& partitioner = dht::global_partitioner();
                return linearized_values.emplace_back(partitioner.token_to_bytes(partitioner.get_token(*_base, base_key)));
            }
