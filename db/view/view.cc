...
std::optional<atomic_cell_view>
get_cell_view_from(const atomic_cell_or_collection* cell_or_collection, const column_definition& cdef, std::optional<bytes_view> collection_key) {
...
void view_updates::do_delete_old_entry(const partition_key& base_key, const clustering_row& existing, const clustering_row& update, gc_clock::time_point now) {
    ...
    if (computed_col) {
        if (auto map_value_computation = dynamic_cast<const map_value_column_computation*>(&computed_col->get_computation())) {
    ...
    if (col_id) {
        ...
        // Note: multi-cell columns can't be part of the primary key.
        auto& def = _base->regular_column_at(*col_id);
        auto cell = get_cell_view_from(&(existing.cells().cell_at(*col_id)), def, collection_key);
        if (cell->is_live()) {
...
void view_updates::generate_update(
    ...
    auto after_cell_view = get_cell_view_from(after, cdef, collection_key);
    if (existing) {
        auto* before = existing->cells().find_cell(*col_id);
        auto before_cell_view = get_cell_view_from(before, cdef, collection_key);
