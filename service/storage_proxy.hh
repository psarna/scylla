...
private:
    ...
    std::vector<gms::inet_address> get_live_endpoints_for_read(keyspace& ks, const dht::token& token, db::consistency_level cl, clock_type::time_point timeout);
    static void sort_endpoints_by_proximity(std::vector<gms::inet_address>& eps);
    std::vector<gms::inet_address> get_live_sorted_endpoints_for_read(keyspace& ks, const dht::token& token, db::consistency_level cl, clock_type::time_point timeout);
...
public:
    ...
    */
    ...
    void mark_responded(const gms::inet_address& ep, clock_type::duration rtt);
