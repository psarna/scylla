...
storage_proxy_stats::split_stats::split_stats(const sstring& category, const sstring& short_description_prefix, const sstring& long_description_prefix, const sstring& op_type)
    ...
    return mutate_prepare(std::array<mutation, 1>{std::move(m)}, db::consistency_level::ONE, type,
        [this, targets = std::move(targets), pending_endpoints = std::move(pending_endpoints)] (
            ...
            return create_write_response_handler(
                    ...
                    { },
                    nullptr);
        }).then([this, &stats] (std::vector<unique_response_handler> ids) {
            return mutate_begin(std::move(ids), db::consistency_level::ONE, stats);
