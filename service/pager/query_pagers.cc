...
static bool has_clustering_keys(const schema& s, const query::read_command& cmd) {
    ...
    future<service::storage_proxy::coordinator_query_result> query_pager::do_fetch_page(uint32_t page_size, gc_clock::time_point now, db::timeout_clock::time_point timeout) {
        ...
        auto max_rows = count_max_rows(page_size);
...
protected:
    virtual uint32_t count_max_rows(uint32_t page_size) override {
        return page_size;
    }
};
...
public:
    ...
    void accept_partition_end(const query::result_row_view& static_row) {
        ...
        if constexpr (std::is_same_v<visitor, cql3::selection::result_set_builder::visitor<cql3::selection::result_set_builder::restrictions_filter>>) {
            dropped_rows += visitor::_filter.get_total_rows_dropped();
        }
    }
};
